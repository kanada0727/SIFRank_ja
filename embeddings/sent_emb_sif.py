...
class SentEmbeddings:
    def __init__(
        ...
        weightfile_pretrain = os.path.join(
            os.path.dirname(__file__),
            "../" "/auxiliary_data/corpus_vocab.txt",
        )

        weightfile_finetune = os.path.join(
            os.path.dirname(__file__),
            "../" "/auxiliary_data/inspec_vocab.txt",
        )
    ...
    def get_tokenized_sent_embeddings(self, text_obj, if_DS=False, if_EA=False):
        ...
        if self.embeddings_type == "elmo" and if_DS == False:
            (
                elmo_embeddings,
                elmo_mask,
            ) = self.word_embeddor.get_tokenized_words_embeddings([text_obj.tokens])
        elif self.embeddings_type == "elmo" and if_DS == True and if_EA == False:
            tokens_segmented = get_sent_segmented(text_obj.tokens)
            (
                elmo_embeddings,
                elmo_mask,
            ) = self.word_embeddor.get_tokenized_words_embeddings(tokens_segmented)
        ...
        for cand_tokens in text_obj.keyphrase_candidate:
...
def get_candidate_weighted_average(
